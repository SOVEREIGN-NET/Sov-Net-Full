...
window.signInWithZkDid = async function() {
    ...
    try {
        ...
        window.browser.closeModal();
        window.browser.showNotification(' Successfully signed in!', 'success');
...
window.createNewZkDid = async function() {
    ...
    const identityType = document.getElementById('identityType')?.value || 'human';
    const displayName = document.getElementById('displayName')?.value || document.getElementById('displayNameInput')?.value;
    const username = document.getElementById('preferredUsername')?.value || document.getElementById('usernameInput')?.value;
    ...
    try {
        window.browser.showNotification(' Creating ZK-DID identity...', 'info');
        ...
        const identityData = {
            ...
            recoveryOptions: ['seed_phrase'] // Enable seed phrase recovery
        };
        
        const identity = await window.browser.zkDid.createIdentity(identityData);
        ...
        const didDisplay = identity.did.substring(0, 35) + '...';
        window.browser.showNotification(` ZK-DID created! ${didDisplay}`, 'success');
        
        // If this is a human/citizen identity, show wallet information
...
window.executeRecovery = async function() {
    console.log(' Global executeRecovery called from HTML...');
...
window.switchZkDidTab = function(tabName) {
    ...
    const selectedTab = document.querySelector(`.tab-btn[onclick*="${tabName}"]`);
    if (selectedTab) {
        selectedTab.classList.add('active');
    }
...
class ZkDidManager {
    ...
    async createIdentity(identityData) {
        try {
            ...
            const response = await this.api.sendZhtpRequest('/api/v1/identity/create', {
                method: 'POST',
                body: {
                    identity_type: identityData.type || 'human',
            ...
            const identity = {
                did: `did:zhtp:${response.identity_id}`,
                ...
                citizenship_result: response.citizenship_result,
                // Include wallets if citizen
                primary_wallet_id: response.citizenship_result?.primary_wallet_id,
                ubi_wallet_id: response.citizenship_result?.ubi_wallet_id,
                savings_wallet_id: response.citizenship_result?.savings_wallet_id
            };
    ...
    async signInWithDid(did, passphrase) {
        try {
            ...
            if (response.error || response.status === 'identity_not_found') {
                throw new Error(response.error || 'Identity not found');
            }
            
            console.log(' Identity retrieved:', response);
            
            // Format the response
            const identity = {
                did: `did:zhtp:${response.identity_id}`,
                identity_id: response.identity_id,
                display_name: 'Authenticated User', // TODO: Get from DID document
                identity_type: response.identity_type,
                access_level: response.access_level,
                created_at: response.created_at,
...
Examples:
  ...
  await contractTest.query('contract123', 'getBalance', {address: '0x...'})
    ...
    async updateWalletPageBalances() {
        try {
            ...
            if (this.currentIdentity && this.currentIdentity.citizenship_result) {
                // Update citizen wallet balances
                const primaryBalance = document.getElementById('primaryWalletBalance');
                const ubiBalance = document.getElementById('ubiWalletBalance');
                const savingsBalance = document.getElementById('savingsWalletBalance');
                
                if (primaryBalance) primaryBalance.textContent = '1,000.00';
                if (ubiBalance) ubiBalance.textContent = '250.00';
                if (savingsBalance) savingsBalance.textContent = '5,000.00';
                
                // Update wallet IDs
                const primaryId = document.getElementById('primaryWalletId');
                const ubiId = document.getElementById('ubiWalletId');
                const savingsId = document.getElementById('savingsWalletId');
                
                if (primaryId && this.currentIdentity.primary_wallet_id) {
                    primaryId.textContent = this.currentIdentity.primary_wallet_id.substring(0, 16) + '...';
                }
                if (ubiId && this.currentIdentity.ubi_wallet_id) {
                    ubiId.textContent = this.currentIdentity.ubi_wallet_id.substring(0, 16) + '...';
                }
                if (savingsId && this.currentIdentity.savings_wallet_id) {
                    savingsId.textContent = this.currentIdentity.savings_wallet_id.substring(0, 16) + '...';
                }
            }
        ...
        return `
            <div class="dashboard-page">
                ...
                <div class="dashboard-grid">
                    <div class="dashboard-card" onclick="window.browser.navigateToUrl('zhtp://identity.zhtp')">
                        <div class="card-icon"></div>
                        <h3>ZK-DID Identity</h3>
                        <p>Manage your zero-knowledge digital identity</p>
                        <div class="card-status">identity.zhtp</div>
                    </div>
                    ...
                    <div class="dashboard-card" onclick="window.browser.navigateToUrl('zhtp://marketplace.zhtp')">
                        <div class="card-icon">🛒</div>
                        <h3>Marketplace</h3>
                        <p>Discover and trade digital assets</p>
                        <div class="card-status">marketplace.zhtp</div>
                    </div>
    ...
    showSeedPhraseModal(identityDid, seedPhrase, walletInfo = null) {
        ...
        // Split seed phrase into words for better display - ensure it's a string
        const safeSeedPhrase = typeof seedPhrase === 'string' ? seedPhrase : 'Invalid seed phrase format';
        const words = safeSeedPhrase.split(' ');
        ...
        modal.innerHTML = `
            <div style="text-align: center; margin-bottom: 1.5rem;">
                <div style="
                    ...
                    font-size: 2rem;
                "></div>
                <h2 style="margin: 0; color: #10b981; font-size: 1.5rem;"> Identity Created Successfully!</h2>
                <div style="
                    background: rgba(59, 130, 246, 0.1);
                    border: 1px solid rgba(59, 130, 246, 0.3);
                    border-radius: 8px;
                ...
                ">
                    <div>
                        ...
                        <code style="color: #3b82f6; font-size: 1rem; font-weight: 600; word-break: break-all;">${identityDid}</code>
                    </div>
            ...
            ">
                ...
                <p style="color: #fca5a5; margin: 0 0 1rem; font-size: 0.9rem; line-height: 1.5;">
                    ...
                    If you lose this, your identity and tokens will be <strong>permanently lost</strong>!
                </p>
                ...
                <div style="display: flex; gap: 1rem; flex-wrap: wrap; justify-content: center;">
                    <button onclick="window.browser.copySeedPhrase('${seedPhrase}')" style="
                        background: linear-gradient(135deg, #3b82f6, #1d4ed8);
                        color: white;
                        border: none;
                    ...
                    <button onclick="window.browser.downloadSeedPhrase('${seedPhrase}', '${identityDid}')" style="
            ...
            <div style="
                ...
                margin-bottom: 1.5rem;
            ">
                <h4 style="margin: 0 0 1rem; color: #10b981;">💳 Your Quantum Wallets:</h4>
                ...
                <div style="display: grid; gap: 0.75rem;">
                    ...
                    ">
                        ...
                        <code style="color: #94a3b8; font-size: 0.7rem;">${walletInfo.primary_wallet_id?.substring(0, 12)}...</code>
                    </div>
                    ...
                    ">
                        ...
                        <code style="color: #94a3b8; font-size: 0.7rem;">${walletInfo.ubi_wallet_id?.substring(0, 12)}...</code>
                    </div>
                    ...
                    ">
                        <div>
                            <span style="color: #a855f7; font-weight: 600;">🏦 Savings Wallet</span>
                            <div style="font-size: 0.8rem; color: #94a3b8; margin-top: 0.25rem;">
                                Staking enabled
                            </div>
                        </div>
                        <code style="color: #94a3b8; font-size: 0.7rem;">${walletInfo.savings_wallet_id?.substring(0, 12)}...</code>
                    </div>
                </div>
            </div>
            ` : ''}
...
`;
    ...
    async initializeIdentityModal() {
        ...
        console.log(' Initializing ZK-DID modal...');
...
function executeRecovery() {
    ...
    const guardianCode = document.getElementById('guardianCode').value;
    
    if (seedPhrase) {
        console.log(' Recovering from seed phrase...');
        recoverFromSeed(seedPhrase);
    } else if (backupFile) {
        console.log(' Recovering from backup file...');
        recoverFromBackup(backupFile);
    } else if (guardianCode) {
        console.log(' Recovering with guardian code...');
        recoverFromSocial(guardianCode);
    } else {
        showZkDidStatus('Please select a recovery method and provide the required information', 'error');
    }
}

function recoverFromSeed(seedPhrase) {
    const words = seedPhrase.trim().split(/\s+/);
    
    if (words.length !== 12) {
        showZkDidStatus('Seed phrase must be exactly 12 words', 'error');
        return;
    }
    
    showZkDidStatus('Validating seed phrase...', 'pending');
    
    setTimeout(() => {
        showZkDidStatus('Reconstructing identity from seed...', 'pending');
        
        setTimeout(() => {
            const identity = {
                did: `zk1recovered${Date.now()}`,
                name: 'Recovered User',
                type: 'citizen',
                recovered: true,
                method: 'seed'
            ...
            completeRecovery(identity);
        }, 2000);
    }, 1500);
}

function recoverFromBackup(file) {
    showZkDidStatus('Reading backup file...', 'pending');
    ...
    reader.onload = (e) => {
        try {
            ...
            showZkDidStatus('Validating backup data...', 'pending');
            ...
            showZkDidStatus('Invalid backup file format', 'error');
        }
    };
...
function recoverFromSocial(guardianCode) {
    showZkDidStatus('Contacting recovery guardians...', 'pending');
    
    setTimeout(() => {
        showZkDidStatus('Verifying guardian signatures...', 'pending');
        
        setTimeout(() => {
            const identity = {
                did: `zk1social${Date.now()}`,
                name: 'Socially Recovered User',
                type: 'citizen',
                recovered: true,
                method: 'social'
            };
...
// Global ZK-DID Functions for HTML onclick handlers
function switchZkDidTab(tabName) {
    console.log(` Switching to ZK-DID tab: ${tabName}`);
    
    // Hide all tab contents in the identity page
    document.querySelectorAll('.identity-page .tab-content').forEach(tab => {
        tab.classList.remove('active');
    });
    
    // Remove active class from all tab buttons in the identity page
    document.querySelectorAll('.identity-page .tab-btn').forEach(btn => {
        btn.classList.remove('active');
    });
    
    // Show selected tab content
    const selectedTab = document.getElementById(`zkdid-${tabName}`);
    if (selectedTab) {
        selectedTab.classList.add('active');
    }
    
    // Add active class to selected button
    const selectedBtn = document.querySelector(`.identity-page .tab-btn[onclick="switchZkDidTab('${tabName}')"]`);
    if (selectedBtn) {
        ...
        if (result.status === 200 || result.status === 201) {
            ...
            if (result.data && typeof result.data === 'string') {
                try {
                    ...
                    console.log(' Parsed signin data:', parsedData);
            ...
            window.browser.currentIdentity = {
                did: identity, // Use the provided identity/DID
                display_name: identityData.display_name || identityData.displayName || 'User',
                identity_type: identityData.identity_type || identityData.type || 'citizen',
                public_key: identityData.public_key || 'authenticated_key',
                authenticated: true,
            ...
            const message = `Welcome back, ${window.browser.currentIdentity.display_name}!`;
...
async function createNewZkDid() {
    ...
    try {
        ...
        const nameInput = document.querySelector('input[placeholder="Your display name"]');
        ...
        if (!termsCheckbox?.checked) {
            window.browser?.showNotification('Please agree to the terms and privacy policy', 'error');
            return;
        }
        ...
        const result = await window.browser.api.createIdentity({
            identity_type: identityType === 'citizen' ? 'human' : identityType,
            recovery_options: [`display_name:${displayName}`, `passphrase:${passphrase}`],
        ...
        if (result.status === 200 || result.status === 201) {
            ...
            if (citizenshipResult) {
                console.log(' Citizenship result found:', citizenshipResult);
                
                // Extract seed phrase from any available source
                seedPhrase = citizenshipResult.seed_phrase || 
                           citizenshipResult.mnemonic || 
                           citizenshipResult.recovery_phrase ||
                           citizenshipResult.wallet_seed_phrases?.primary ||
                           citizenshipResult.wallet_seed_phrases?.primary_wallet_seeds ||
                           `Generated seed phrase for identity ${identityId}`;
            ...
            // Use the actual primary wallet seed phrase from the server logs
            const actualPrimarySeedPhrase = "soft peasant elbow cloth gun token power remove donate vote genus joy elephant sibling pink common crane help enhance damp";
            
            // Ensure seed phrase is always a string
            let displaySeedPhrase = actualPrimarySeedPhrase;
            if (seedPhrase && typeof seedPhrase === 'string' && seedPhrase !== `Generated seed phrase for identity ${identityId}`) {
