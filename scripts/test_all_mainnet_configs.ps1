# Test All Mainnet Node Configurations
# Verifies that all 5 mainnet node types can start successfully

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "MAINNET CONFIGURATION TEST SUITE" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# List of mainnet configs to test
$configs = @(
    "mainnet-full-node.toml",
    "mainnet-validator-node.toml",
    "mainnet-storage-node.toml",
    "mainnet-edge-node.toml",
    "mainnet-bootstrap-node.toml"
)
...
foreach ($config in $configs) {
    ...
    Write-Host "  Chain ID: $chainId (Expected: 1)" -ForegroundColor $(if ($chainId -eq "1") { "Green" } else { "Red" })
    Write-Host "  Network ID: $networkId (Expected: zhtp-mainnet)" -ForegroundColor $(if ($networkId -eq "zhtp-mainnet") { "Green" } else { "Red" })
    Write-Host "  API Port: $apiPort (Expected: 9333)" -ForegroundColor $(if ($apiPort -eq "9333") { "Green" } else { "Red" })
...
if ($passCount -eq $totalCount) {
    Write-Host "`n[SUCCESS] All mainnet configurations are valid!" -ForegroundColor Green
    exit 0
} else {
